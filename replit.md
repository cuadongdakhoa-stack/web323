...
- **CrCl Auto-Calculation**: Automatically calculates Creatinine Clearance (CrCl) using the **Cockcroft-Gault formula** when serum creatinine, age, gender, and weight are provided. Classifies renal function into G1-G5 categories with Vietnamese descriptions. Formula: Males: CrCl = [(140 - age) × weight] / (72 × SCr); Females: CrCl = [(140 - age) × weight × 0.85] / (72 × SCr) where SCr is in mg/dL. Results include CrCl value (mL/min - not normalized to BSA), category (G1-G5), and Vietnamese renal function description.
