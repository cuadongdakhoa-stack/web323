...
- **CrCl Auto-Calculation**: Automatically calculates Creatinine Clearance (CrCl) using the **Cockcroft-Gault formula** when serum creatinine, age, gender, and weight are provided. Classifies renal function into G1-G5 categories with Vietnamese descriptions. **Formula**: Males: CrCl = [(140 - age) × weight] / (72 × SCr); Females: CrCl = [(140 - age) × weight × 0.85] / (72 × SCr) where SCr is in mg/dL. **Unit conversion**: µmol/L ÷ 88.4 = mg/dL (per Cockcroft-Gault reference). Results include CrCl value (mL/min - not normalized to BSA), category (G1-G5), and Vietnamese renal function description.
...
- **Chat AI Assistant**: A context-aware chatbot for clinical pharmacy questions. **Enhanced with hospital-wide intelligence**: AI now has access to real-time system statistics (total cases, total patients, top 5 diagnoses, top 10 medications) via cached aggregated SQL queries (5-minute TTL). **Personality**: Responds like a knowledgeable hospital employee with detailed explanations, structured responses (bullet points, sections), and reasoning. **Temperature: 0.4** (reduced from 0.7) for more consistent answers. **UI**: Responsive design with `max-w-[calc(100vw-3rem)]` on mobile and adaptive height `h-[min(600px,80vh)]`. Implementation: `client/src/components/floating-chatbot.tsx`, `server/openrouter.ts` (chatWithAI function), `server/storage.ts` (getSystemStatistics with O(1) memory-safe aggregation).
- **Multi-file Upload & AI Extraction**: Supports uploading up to 10 files (PDF, DOCX, JPG, PNG) per case, with duplicate prevention and partial failure handling. **Intelligent data merging**: Extracts patient demographics (name, age, gender, weight, height, admission date), clinical data (creatinine + unit, diagnoses with ICD-10, medical history, allergies), and medications from each file, then merges using smart priority logic (new non-null values override previous values). **Batch processing**: Handles multiple files sequentially, accumulates extracted data, and applies comprehensive field mapping including `labResults.creatinine`, `labResults.creatinineUnit`, and `admissionDate`. **Enhanced table recognition**: AI extraction now specifically recognizes Vietnamese medical forms with numbered fields (e.g., "(15) Chẩn đoán xác định", "(16) Mã bệnh", "(17) Bệnh kèm theo", "(18) Mã bệnh kèm theo") and correctly parses all diagnosis + ICD-10 codes including secondary diagnoses separated by semicolons or commas. Uses mammoth library for DOCX text extraction and pdfjs-dist for PDF parsing.
- **Structured Diagnosis**: Integrates ICD-10 codes for main and secondary diagnoses, with AI extraction support. **Full extraction coverage**: AI now extracts ALL diagnoses and ICD codes from medical forms, including bảng kê (numbered list format) commonly used in Vietnamese hospitals.
...
- **Drug Formulary Management**: Hospital-specific drug database with trade names, active ingredients, strengths, units, and manufacturers. **Admin-only upload** via Excel/CSV (supports both Vietnamese and English column headers, accepts .xlsx, .xls, and .csv formats, max 10MB). **Dual-path upload logic**: Files <5MB use fast XLSX.sheet_to_json() parsing; files ≥5MB (up to 10MB limit) automatically trigger AI extraction via DeepSeek (temperature 0.3) for complex/corrupted files that standard parsers cannot handle. **Features**: (1) Case-insensitive search (`ilike`) for Vietnamese drug names, (2) AI integration: formulary data automatically enriches medication context in clinical analysis prompts, (3) Row-level validation with Zod schema prevents data corruption, (4) Nullish coalescing preserves zero-strength values during import, (5) Read access for all authenticated users, write access restricted to admin role, (6) Comprehensive error handling with user-friendly 400 errors for corrupt/empty files, (7) Request body limits set to 10MB (matching file upload limit). Implementation in `server/storage.ts`, `server/routes.ts`, `server/openrouter.ts` (extractDrugDataFromFile function), and `client/src/pages/drug-formulary.tsx`.
